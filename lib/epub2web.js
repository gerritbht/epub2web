...
module.exports.webify = function webify(epubfile, templateName, cb) {
	...
	cacher.cache(epubfile, function (err, cacheId, bundle) {
		...
		console.log('debug->2web->cached epub file '+epubfile+'. cacheId is '+cacheId);

		//var config = JSON.stringify(bundle);
		config = JSON.stringify(bundle);

		//console.log('epub2web config = ', config); //debug output, here it WORKS
		...
		//console.log('webify out ' + cacheId + ' and html ' + html); //debug output
...
module.exports.reader = function reader(cacheId, templateName, cb) {
	...
	try {
		...
		console.log("Error in reader function");
		console.log(e);
		cb(e,null, null);
	}
}
